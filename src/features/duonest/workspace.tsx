...
import { AlertCircle, Bell, CalendarDays, Check, CheckCircle2, CircleDollarSign, ClipboardList, Clock, Edit3, History, Home, ListChecks, LoaderCircle, Plus, Repeat2, ShoppingCart, Sparkles, Target, Trash2, Trophy, Users, WalletCards, X } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Progress } from "@/components/ui/progress";
import { Avatar } from "@/components/ui/avatar";
import { BrandLogo } from "@/components/brand/brand-logo";
import { cn, formatPercent } from "@/lib/utils";
import type { AttentionPoint, HomeRecord, HouseholdExpense, HouseholdPlan, Mission, ShoppingItem, Task, WorkspaceData } from "@/lib/duonest/types";
import type { DuoNestView } from "./duonest-app";
import { AvatarUpload } from "./avatar-upload";
import { BrowserNotificationControl } from "./browser-notifications";
import {
  createAttentionPoint, createExpense, createInvite, createPlan, createRecord, createShoppingItem, createTask,
  deleteAttentionPoint, deleteExpense, deletePlan, deleteRecord, deleteShoppingItem, deleteTask, markNotification, removeMember,
  resolveAttentionPoint, toggleShoppingItem, toggleTask, updateAttentionPoint, updateHome, updateProfile,
  toggleExpense, updatePlanStatus, updateExpense, updatePlan, updateRecord, updateShoppingItem, updateTask, type ActionResult,
} from "./actions";

type Editor = { kind: "task"; item?: Task } | { kind: "shopping"; item?: ShoppingItem } | { kind: "attention"; item?: AttentionPoint } | { kind: "record"; item?: HomeRecord } | { kind: "expense"; item?: HouseholdExpense } | { kind: "plan"; item?: HouseholdPlan } | null;

const titleByView: Record<DuoNestView, string> = {
  dashboard: "Visão geral", tarefas: "Tarefas", calendario: "Calendário", missoes: "Missões inteligentes", mercado: "Mercado",
  financeiro: "Financeiro", planejamentos: "Planejamentos", historico: "Histórico compartilhado",
...
const modalKindByView: Partial<Record<DuoNestView, NonNullable<Editor>["kind"]>> = { tarefas: "task", mercado: "shopping", pontos: "attention", registros: "record", financeiro: "expense", planejamentos: "plan" };
...
export function Workspace({ view, data, initialOpen }: { view: DuoNestView; data: WorkspaceData; initialOpen: boolean }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {view === "financeiro" ? <Finance data={data} run={run} edit={(item) => setEditor({ kind: "expense", item })} remove={(item) => confirmDelete(`o gasto “${item.description}”`, deleteExpense(item.id))} /> : null}
      {view === "planejamentos" ? <Plans data={data} run={run} edit={(item) => setEditor({ kind: "plan", item })} remove={(item) => confirmDelete(`o planejamento “${item.title}”`, deletePlan(item.id))} /> : null}
        ...
        const action = editor.kind === "task" ? (editor.item ? updateTask(formData) : createTask(formData))
          : editor.kind === "shopping" ? (editor.item ? updateShoppingItem(formData) : createShoppingItem(formData))
            : editor.kind === "attention" ? (editor.item ? updateAttentionPoint(formData) : createAttentionPoint(formData))
              : editor.kind === "record" ? (editor.item ? updateRecord(formData) : createRecord(formData))
                : editor.kind === "expense" ? (editor.item ? updateExpense(formData) : createExpense(formData))
                  : (editor.item ? updatePlan(formData) : createPlan(formData));
        run(action, true);
      }} /> : null}
    </div>
  );
...
function Dashboard({ data, run }: { data: WorkspaceData; run: (action: Promise<ActionResult>) => void }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[1.4fr_0.9fr]">
    ...
    <div className="space-y-5">
      <Surface><SectionTitle icon={Sparkles} title="Missões inteligentes" href="/missoes" />{data.missions.slice(0, 3).map((mission) => <MissionRow key={mission.id} mission={mission} completed={data.missionCompletionIds.includes(mission.id)} data={data} />)}{!data.missions.length ? <Empty icon={Sparkles} title="Analisando sua rotina" text="As próximas metas serão geradas pelo seu progresso." /> : null}</Surface>
...
function Tasks({ data, run, edit, remove }: { data: WorkspaceData; run: (a: Promise<ActionResult>) => void; edit: (t: Task) => void; remove: (t: Task) => void }) {
  return <Surface>{data.tasks.length ? <div className="divide-y divide-slate-100 dark:divide-neutral-800">{data.tasks.map((task) => { const overdue = isTaskOverdue(task); const canExtend = overdue && task.created_by === data.userId; return <div key={task.id} className="flex items-center gap-2 py-3"><TaskRow task={task} data={data} toggle={() => run(toggleTask(task.id, task.status !== "resolved"))} className="flex-1" />{!overdue || canExtend ? <IconButton label={canExtend ? "Prorrogar prazo" : "Editar tarefa"} onClick={() => edit(task)}>{canExtend ? <CalendarDays size={17} /> : <Edit3 size={17} />}</IconButton> : null}<IconButton label="Excluir tarefa" danger onClick={() => remove(task)}><Trash2 size={17} /></IconButton></div>; })}</div> : <Empty icon={ListChecks} title="Nenhuma tarefa" text="Adicione a primeira tarefa da casa." />}</Surface>;
}
...
function TaskRow({ task, data, toggle, className }: { task: Task; data: WorkspaceData; toggle: () => void; className?: string }) {
  ...
  const responsibles = task.assigned_to
    ...
    : task.assignment === "self"
      ...
      : task.assignment === "partner"
        ? data.members.filter((item) => item.user_id !== data.userId).slice(0, 1)
        : data.members;
  ...
  return <button onClick={toggle} disabled={overdue} title={overdue ? "O criador precisa prorrogar o prazo antes da conclusão" : undefined} className={cn("flex min-w-0 items-center gap-3 rounded-lg p-2 text-left hover:bg-slate-50 disabled:cursor-not-allowed disabled:bg-red-50/60 dark:hover:bg-neutral-800 dark:disabled:bg-red-950/20", className)}><span className={cn("grid size-8 shrink-0 place-items-center rounded-full border", task.status === "resolved" ? "border-emerald-600 bg-emerald-600 text-white" : overdue ? "border-red-400 text-red-600" : "border-slate-300 dark:border-neutral-600")}><Check size={16} className={task.status === "resolved" ? "" : "opacity-0"} /></span><span className="min-w-0 flex-1"><span className={cn("block truncate font-medium", task.status === "resolved" && "text-slate-400 line-through")}>{task.title}</span><span className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500 dark:text-neutral-400"><span>{task.category}</span><span className={cn("inline-flex items-center gap-1", overdue && "font-semibold text-red-600 dark:text-red-400")}><CalendarDays size={13} />{overdue ? "Prazo vencido: conclusão bloqueada" : task.due_at ? `Prazo ${dateLabel(task.due_at, true)}` : "Sem prazo"}</span>{task.recurrence !== "none" ? <span className="inline-flex items-center gap-1"><Repeat2 size={13} />{recurrenceLabel(task.recurrence)}</span> : null}<span className="inline-flex min-w-0 items-center gap-1.5"><span className="flex shrink-0 -space-x-1">{responsibles.slice(0, 3).map((item) => <Avatar key={item.id} name={item.profile.display_name} src={item.profile.avatar_url} size={20} className="ring-2 ring-white dark:ring-neutral-900" />)}</span><span className="truncate">{responsibleLabel}</span></span><span className="font-semibold text-emerald-700 dark:text-emerald-400">+{task.xp} XP</span><span className="font-semibold text-red-600 dark:text-red-400">-{task.xp} se atrasar</span></span></span><Badge tone={overdue || task.priority === "urgent" ? "red" : task.priority === "high" ? "amber" : "neutral"}>{overdue ? "Bloqueada" : priorityLabel(task.priority)}</Badge></button>;
}
...
function Finance({ data, run, edit, remove }: { data: WorkspaceData; run: (a: Promise<ActionResult>) => void; edit: (i: HouseholdExpense) => void; remove: (i: HouseholdExpense) => void }) {
  const [month, setMonth] = useState(new Date().toISOString().slice(0, 7));
  const expenses = data.expenses.filter((item) => item.expense_date.startsWith(month));
  const total = expenses.reduce((sum, item) => sum + Number(item.amount), 0);
  const paid = expenses.filter((item) => item.status === "paid").reduce((sum, item) => sum + Number(item.amount), 0);
  return <div className="space-y-5"><div className="grid gap-3 sm:grid-cols-3"><FinanceMetric label="Total do mês" value={total} icon={WalletCards} /><FinanceMetric label="Pago" value={paid} icon={CheckCircle2} /><FinanceMetric label="Pendente" value={total - paid} icon={Clock} /></div><Surface><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="font-semibold">Gastos do mês</h2><label className="text-sm font-medium">Mês<input aria-label="Mês dos gastos" type="month" value={month} onChange={(event) => setMonth(event.target.value)} className="ml-2 h-10 rounded-lg border border-slate-300 bg-white px-3 dark:border-neutral-700 dark:bg-neutral-950" /></label></div>{expenses.length ? <div className="mt-4 divide-y divide-slate-100 dark:divide-neutral-800">{expenses.map((item) => { const payer = data.members.find((member) => member.user_id === item.paid_by); return <div key={item.id} className="flex items-center gap-3 py-3"><button onClick={() => run(toggleExpense(item.id, item.status !== "paid"))} className={cn("grid size-8 shrink-0 place-items-center rounded-full border", item.status === "paid" ? "border-emerald-600 bg-emerald-600 text-white" : "border-slate-300")} aria-label={item.status === "paid" ? "Marcar como pendente" : "Marcar como pago"}>{item.status === "paid" ? <Check size={16} /> : null}</button><div className="min-w-0 flex-1"><p className="truncate font-medium">{item.description}</p><p className="mt-1 flex flex-wrap items-center gap-2 text-xs text-slate-500"><span>{item.category}</span><span>{dateLabel(item.expense_date)}</span>{payer ? <span className="inline-flex items-center gap-1"><Avatar name={payer.profile.display_name} src={payer.profile.avatar_url} size={18} />{payer.profile.display_name}</span> : null}</p></div><strong className="shrink-0 text-sm">{money(item.amount)}</strong><IconButton label="Editar gasto" onClick={() => edit(item)}><Edit3 size={17} /></IconButton><IconButton label="Excluir gasto" danger onClick={() => remove(item)}><Trash2 size={17} /></IconButton></div>; })}</div> : <Empty icon={CircleDollarSign} title="Nenhum gasto neste mês" text="Registre contas, compras e despesas compartilhadas." />}</Surface></div>;
}

function FinanceMetric({ label, value, icon: Icon }: { label: string; value: number; icon: typeof Home }) {
  return <Surface className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-lg bg-emerald-50 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300"><Icon size={20} /></span><span><small className="text-slate-500">{label}</small><strong className="block text-lg">{money(value)}</strong></span></Surface>;
}

function Plans({ data, run, edit, remove }: { data: WorkspaceData; run: (a: Promise<ActionResult>) => void; edit: (i: HouseholdPlan) => void; remove: (i: HouseholdPlan) => void }) {
  return data.plans.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{data.plans.map((plan) => { const responsible = data.members.find((member) => member.user_id === plan.responsible_id); return <Surface key={plan.id}><div className="flex items-start justify-between gap-3"><Badge tone={plan.status === "completed" ? "green" : plan.status === "cancelled" ? "red" : plan.status === "in_progress" ? "blue" : "neutral"}>{planStatusLabel(plan.status)}</Badge><div className="flex"><IconButton label="Editar planejamento" onClick={() => edit(plan)}><Edit3 size={17} /></IconButton><IconButton label="Excluir planejamento" danger onClick={() => remove(plan)}><Trash2 size={17} /></IconButton></div></div><ClipboardList className="mt-4 text-emerald-700 dark:text-emerald-400" size={22} /><h2 className="mt-3 text-lg font-semibold">{plan.title}</h2><p className="mt-2 min-h-10 text-sm text-slate-500 dark:text-neutral-400">{plan.description || planTypeLabel(plan.plan_type)}</p><div className="mt-4 space-y-2 text-sm"><p>{plan.target_date ? `Meta: ${dateLabel(plan.target_date)}` : "Sem data definida"}</p>{plan.estimated_cost !== null ? <p className="font-semibold">Estimativa: {money(plan.estimated_cost)}</p> : null}{responsible ? <p className="flex items-center gap-2 text-slate-500"><Avatar name={responsible.profile.display_name} src={responsible.profile.avatar_url} size={24} />{responsible.profile.display_name}</p> : null}</div><select aria-label={`Andamento de ${plan.title}`} value={plan.status} onChange={(event) => run(updatePlanStatus(plan.id, event.target.value as HouseholdPlan["status"]))} className="mt-5 h-10 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm dark:border-neutral-700 dark:bg-neutral-950"><option value="planned">Planejado</option><option value="in_progress">Em andamento</option><option value="completed">Concluído</option><option value="cancelled">Cancelado</option></select></Surface>; })}</div> : <Surface><Empty icon={ClipboardList} title="Nenhum planejamento" text="Organize compras, melhorias e manutenções da casa." /></Surface>;
}

function HistoryView({ data }: { data: WorkspaceData }) {
  const completedIds = new Set(data.taskCompletions.map((item) => item.task_id));
  const completed = data.tasks.filter((task) => completedIds.has(task.id) || task.status === "resolved");
  const pending = data.tasks.filter((task) => !completedIds.has(task.id) && task.status !== "resolved");
  return <div className="grid gap-5 xl:grid-cols-[1fr_1fr]"><div className="space-y-5"><Surface><SectionTitle icon={CheckCircle2} title={`Concluídas (${completed.length})`} /><div className="mt-3 space-y-2">{completed.map((task) => <HistoryTask key={task.id} task={task} data={data} done />)}{!completed.length ? <p className="text-sm text-slate-500">Nenhuma tarefa concluída.</p> : null}</div></Surface><Surface><SectionTitle icon={Clock} title={`Não concluídas (${pending.length})`} /><div className="mt-3 space-y-2">{pending.map((task) => <HistoryTask key={task.id} task={task} data={data} />)}{!pending.length ? <p className="text-sm text-slate-500">Nenhuma tarefa pendente.</p> : null}</div></Surface></div><Surface><SectionTitle icon={History} title="Todas as ações" /><ActivityList data={data} /></Surface></div>;
}

function HistoryTask({ task, data, done = false }: { task: Task; data: WorkspaceData; done?: boolean }) {
  const completion = data.taskCompletions.find((item) => item.task_id === task.id);
  const responsible = data.members.find((member) => member.user_id === (completion?.completed_by ?? task.assigned_to));
  return <div className="flex items-center gap-3 rounded-lg border border-slate-200 p-3 dark:border-neutral-700"><span className={cn("grid size-8 place-items-center rounded-full", done ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-950" : isTaskOverdue(task) ? "bg-red-100 text-red-700 dark:bg-red-950" : "bg-slate-100 text-slate-500 dark:bg-neutral-800")} >{done ? <Check size={16} /> : <Clock size={16} />}</span><span className="min-w-0 flex-1"><strong className="block truncate text-sm">{task.title}</strong><small className="text-slate-500">{completion ? `Concluída em ${dateLabel(completion.completed_at, true)}` : isTaskOverdue(task) ? "Prazo vencido" : task.due_at ? `Prazo ${dateLabel(task.due_at, true)}` : "Sem prazo"}</small></span>{responsible ? <Avatar name={responsible.profile.display_name} src={responsible.profile.avatar_url} size={30} /> : null}</div>;
}

function Missions({ data }: { data: WorkspaceData }) {
  return data.missions.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{data.missions.map((mission) => { const completed = data.missionCompletionIds.includes(mission.id); const member = data.members.find((item) => item.user_id === mission.assigned_to); const target = mission.target_count ?? 1; return <Surface key={mission.id}><div className="flex items-start justify-between gap-3"><Badge tone={completed ? "green" : mission.mission_type === "daily" ? "blue" : "neutral"}>{completed ? "Concluída" : missionTypeLabel(mission.mission_type)}</Badge>{member ? <span className="flex min-w-0 items-center gap-2 text-xs text-slate-500"><Avatar name={member.profile.display_name} src={member.profile.avatar_url} size={28} /><span className="truncate">{member.profile.display_name}</span></span> : null}</div><Target className="mt-5 text-emerald-700 dark:text-emerald-400" size={22} /><h2 className="mt-3 text-lg font-semibold">{mission.name}</h2><p className="mt-2 min-h-10 text-sm text-slate-500 dark:text-neutral-400">{mission.description}</p><div className="mt-5 flex items-center justify-between text-xs"><span className="font-semibold">{Math.min(mission.progress_count, target)} de {target}</span><span className="font-semibold text-emerald-700 dark:text-emerald-400">+{mission.xp} XP</span></div><Progress value={formatPercent(mission.progress_count, target)} className="mt-2" /><div className="mt-4 flex items-center gap-1 text-xs text-slate-500"><CalendarDays size={13} />Até {mission.due_at ? dateLabel(mission.due_at, true) : "o fim do ciclo"}</div></Surface>; })}</div> : <Surface><Empty icon={Sparkles} title="Analisando sua rotina" text="As missões aparecerão conforme você usar o DuoNest." /></Surface>;
}

function MissionRow({ mission, completed, data }: { mission: Mission; completed: boolean; data: WorkspaceData }) {
  const member = data.members.find((item) => item.user_id === mission.assigned_to);
  const target = mission.target_count ?? 1;
  return <div className="mt-3 rounded-lg border border-slate-200 p-3 dark:border-neutral-700"><div className="flex items-center gap-3"><span className={cn("grid size-8 shrink-0 place-items-center rounded-lg", completed ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-950" : "bg-amber-100 text-amber-700 dark:bg-amber-950")}><Sparkles size={17} /></span><span className="min-w-0 flex-1"><strong className="block truncate text-sm">{mission.name}</strong><small className="text-slate-500 dark:text-neutral-400">{Math.min(mission.progress_count, target)}/{target} · +{mission.xp} XP</small></span>{member ? <Avatar name={member.profile.display_name} src={member.profile.avatar_url} size={28} /> : null}</div><Progress value={formatPercent(mission.progress_count, target)} className="mt-3" /></div>;
}
...
function ExpenseFields({ item, members }: { item?: HouseholdExpense; members: WorkspaceData["members"] }) { return <><Field name="description" label="Descrição" defaultValue={item?.description} required /><div className="grid gap-4 sm:grid-cols-2"><Field name="amount" label="Valor (R$)" type="number" step="0.01" defaultValue={item ? String(item.amount) : ""} required /><Field name="expenseDate" label="Data" type="date" defaultValue={item?.expense_date ?? new Date().toISOString().slice(0,10)} required /></div><div className="grid gap-4 sm:grid-cols-2"><Field name="category" label="Categoria" defaultValue={item?.category ?? "Outros"} /><Select name="status" label="Situação" defaultValue={item?.status ?? "pending"} options={[["pending","Pendente"],["paid","Pago"]]} /></div><Select name="paidBy" label="Pago por" defaultValue={item?.paid_by ?? ""} options={[["","Não definido"], ...members.map((member) => [member.user_id, member.profile.display_name])]} /><TextArea name="notes" label="Observações" defaultValue={item?.notes} /></>; }
function PlanFields({ item, members }: { item?: HouseholdPlan; members: WorkspaceData["members"] }) { return <><Field name="title" label="Título" defaultValue={item?.title} required /><TextArea name="description" label="Descrição" defaultValue={item?.description} /><div className="grid gap-4 sm:grid-cols-2"><Select name="planType" label="Tipo" defaultValue={item?.plan_type ?? "other"} options={[["purchase","Compra"],["improvement","Melhoria"],["maintenance","Manutenção"],["other","Outro"]]} /><Select name="status" label="Andamento" defaultValue={item?.status ?? "planned"} options={[["planned","Planejado"],["in_progress","Em andamento"],["completed","Concluído"],["cancelled","Cancelado"]]} /></div><div className="grid gap-4 sm:grid-cols-2"><Field name="targetDate" label="Data alvo" type="date" defaultValue={item?.target_date ?? ""} /><Field name="estimatedCost" label="Custo estimado (R$)" type="number" defaultValue={item?.estimated_cost !== null && item?.estimated_cost !== undefined ? String(item.estimated_cost) : ""} /></div><Select name="responsibleId" label="Responsável" defaultValue={item?.responsible_id ?? ""} options={[["","Toda a casa"], ...members.map((member) => [member.user_id, member.profile.display_name])]} /></>; }

function Field({ name, label, type = "text", defaultValue, placeholder, required, disabled, step }: { name: string; label: string; type?: string; defaultValue?: string; placeholder?: string; required?: boolean; disabled?: boolean; step?: string }) { return <label className="block text-sm font-medium">{label}<input name={name} type={type} step={step} defaultValue={defaultValue} placeholder={placeholder} required={required} disabled={disabled} className="mt-1 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/15 disabled:opacity-60 dark:border-neutral-700 dark:bg-neutral-950" /></label>; }
...
function submitForm(event: FormEvent<HTMLFormElement>, submit: (data: FormData) => void) { event.preventDefault(); const formData = new FormData(event.currentTarget); formData.set("timezoneOffset", String(new Date().getTimezoneOffset())); submit(formData); }
...
function recurrenceLabel(value: Task["recurrence"]) { return ({ none: "Sem renovação", daily: "Todos os dias", weekly: "Toda semana", biweekly: "A cada 15 dias", monthly: "Todo mês", custom: "Personalizada" })[value]; }
...
function planTypeLabel(value: HouseholdPlan["plan_type"]) { return ({ purchase: "Compra", improvement: "Melhoria", maintenance: "Manutenção", other: "Outro" })[value]; }
function planStatusLabel(value: HouseholdPlan["status"]) { return ({ planned: "Planejado", in_progress: "Em andamento", completed: "Concluído", cancelled: "Cancelado" })[value]; }
function isTaskOverdue(task: Task) { return task.status !== "resolved" && Boolean(task.due_at && new Date(task.due_at).getTime() < Date.now()); }
