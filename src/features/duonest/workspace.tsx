...
function TaskRow({ task, data, toggle, className }: { task: Task; data: WorkspaceData; toggle: () => void; className?: string }) {
  ...
  return <button onClick={toggle} className={cn("flex min-w-0 items-center gap-3 rounded-lg p-2 text-left hover:bg-slate-50 dark:hover:bg-neutral-800", className)}><span className={cn("grid size-8 shrink-0 place-items-center rounded-full border", task.status === "resolved" ? "border-emerald-600 bg-emerald-600 text-white" : "border-slate-300 dark:border-neutral-600")}><Check size={16} className={task.status === "resolved" ? "" : "opacity-0"} /></span><span className="min-w-0 flex-1"><span className={cn("block truncate font-medium", task.status === "resolved" && "text-slate-400 line-through")}>{task.title}</span><span className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500 dark:text-neutral-400"><span>{task.category}</span><span className="inline-flex items-center gap-1"><CalendarDays size={13} />{task.due_at ? `Prazo ${dateLabel(task.due_at, true)}` : "Sem prazo"}</span><span className="inline-flex min-w-0 items-center gap-1.5"><span className="flex shrink-0 -space-x-1">{responsibles.slice(0, 3).map((item) => <Avatar key={item.id} name={item.profile.display_name} src={item.profile.avatar_url} size={20} className="ring-2 ring-white dark:ring-neutral-900" />)}</span><span className="truncate">{responsibleLabel}</span></span></span></span><Badge tone={task.priority === "urgent" ? "red" : task.priority === "high" ? "amber" : "neutral"}>{priorityLabel(task.priority)}</Badge><span className="hidden text-sm font-semibold text-emerald-700 dark:text-emerald-400 sm:block">+{task.xp}</span></button>;
}
